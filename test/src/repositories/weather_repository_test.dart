...
import 'package:daygraph/src/api/api.dart';
import 'package:daygraph/src/domain/entities/weather/weather.dart';
...
void main() {
  ...
  test('repository with mocked http client, failure 401', () async {
    final mockHttpClient = MockHttpClient();
    final api = OpenWeatherMapAPI('apiKey');
    final weatherRepository =
        HttpWeatherRepository(api: api, client: mockHttpClient);
  ...
  test('repository with mocked http client, failure 404', () async {
    final mockHttpClient = MockHttpClient();
    final api = OpenWeatherMapAPI('apiKey');
    final weatherRepository =
    HttpWeatherRepository(api: api, client: mockHttpClient);
    when(() => mockHttpClient.get(api.weather('Seoul')))
        .thenAnswer((_) => Future.value(http.Response('{}', 404)));
    // expectations
    expect(() => weatherRepository.getWeather(city: 'Seoul'),
        throwsA(isA<APIError>()));
  });

  test('repository with mocked http client, failure 500', () async {
    final mockHttpClient = MockHttpClient();
    final api = OpenWeatherMapAPI('apiKey');
    final weatherRepository =
    HttpWeatherRepository(api: api, client: mockHttpClient);
